...
.. container:: ap-lang ap-lang-zh
   ...
   把粒子单位权重按三维 CIC 权重沉积到 3D Cartesian density 数组。
   ...
   ``sub_B01_scatter_3Dxyz`` 把 ``np`` 个粒子的单位贡献按三维 CIC 权重沉积到
   ``den`` 的八个相邻节点，并在所有粒子处理完成后把整个 ``den`` 乘以 ``w``。
   输入粒子位置 ``par(1:3,p)`` 使用网格单位，参考节点由 ``floor`` 得到。
   ...
   .. list-table::
      ...
      * - ``il(1:3)``, ``iu(1:3)``
        - in
        - 本 rank 或本局部网格的有效索引下界和上界。
      * - ``den``
        - in/out
        - 带一层 ghost cell 的沉积数组；调用前通常应清零。
      * - ``np``, ``par(1:6,1:np)``
        - in
        - 粒子数和粒子数组，其中 ``par(1:3,p)`` 为位置。
      * - ``w``
        - in
        - 沉积后整体乘上的权重或归一化因子。
...
.. container:: ap-lang ap-lang-en
   ...
   Deposits unit particle weights to a 3D Cartesian density array with CIC weights.
   ...
   ``sub_B01_scatter_3Dxyz`` deposits unit contribution from ``np`` particles
   to the eight neighboring nodes of ``den`` with three-dimensional CIC
   weights, then multiplies the whole ``den`` array by ``w``. Particle
   positions ``par(1:3,p)`` are in grid units and the reference node is found
   with ``floor``.

   .. rubric:: Generated API

   .. doxygenfile:: sub_B01_scatter_3Dxyz.f90
