------------------------------------
sub_B02_average_axis_jz_3d_cyl.f90
------------------------------------
...
.. container:: ap-lang ap-lang-zh
   ...
   ``sub_B02_average_axis_jz_3d_cyl`` 对柱坐标轴线 ``r=0`` 上的轴向电流密度
   ``Jz`` 做方位向平均。它应在所有粒子的电流沉积完成后调用，使
   ``jz(0,j,k)`` 的所有 ``j`` 使用同一个物理轴线值。

.. container:: ap-lang ap-lang-en

   .. rubric:: Direct Purpose

   Azimuthally averages axial-current values on the cylindrical axis.
   ...
   ``sub_B02_average_axis_jz_3d_cyl`` averages axial current density ``Jz`` on
   the cylindrical axis ``r=0`` over the azimuthal direction. It should be
   called after all particle current deposition so every ``jz(0,j,k)`` entry
   uses one shared physical axis value.

   .. rubric:: Generated API

   .. doxygenfile:: sub_B02_average_axis_jz_3d_cyl.f90
