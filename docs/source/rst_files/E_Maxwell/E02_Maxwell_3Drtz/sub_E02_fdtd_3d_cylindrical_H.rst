sub_E02_fdtd_3d_cylindrical_H.f90
----------------------------------
...
.. container:: ap-lang ap-lang-zh
   ...
   在三维柱坐标 ``r-phi-z`` Yee 网格上，用 ``Er/Ephi/Ez`` 更新 ``Hr/Hphi/Hz``。
   ...
   - 分别更新 ``Hr``、``Hphi``、``Hz`` 三个磁场分量。
   - ``Hr`` 在 ``i=0`` 处由 ``Hphi`` 闭合；``Hz`` 使用含 ``r Ephi`` 的柱坐标径向项。
...
.. container:: ap-lang ap-lang-en
   ...
   Updates ``Hr/Hphi/Hz`` from ``Er/Ephi/Ez`` on a 3D cylindrical ``r-phi-z`` Yee grid.
   ...
   - Updates ``Hr``, ``Hphi``, and ``Hz`` in separate loops.
   - The ``Hr`` axis branch is closed from ``Hphi``; ``Hz`` uses the cylindrical radial term involving ``r Ephi``.
   ...
   .. doxygenfile:: sub_E02_fdtd_3d_cylindrical_H.f90
