...
.. container:: ap-lang ap-lang-zh
   ...
   这个模块提供 E01 TEz 分量组的 CPML 版本 ``Ephi`` 与 ``Hr/Hz`` 更新例程。
   ...
   .. list-table::
      ...
      * - 入口 / 文件
        - 角色
        - 关系
      * - ``sub_E01_cpml_2d_rz_tez_E``
        - 用 ``Hr/Hz`` 更新 ``Ephi`` 的 CPML 电场入口。
        - 在本 module 文件的 ``contains`` 段中定义。
      * - ``sub_E01_cpml_2d_rz_tez_H``
        - 用 ``Ephi`` 更新 ``Hr/Hz`` 的 CPML 磁场入口。
        - 在本 module 文件的 ``contains`` 段中定义。
...
.. container:: ap-lang ap-lang-en
   ...
   This module provides CPML update routines for the E01 TEz component group,
   namely the ``Ephi`` electric update and the ``Hr/Hz`` magnetic update.
   ...
   .. list-table::
      ...
      * - Entry / File
        ...
        - Relation
      * - ``sub_E01_cpml_2d_rz_tez_E``
        - CPML electric-field entry that updates ``Ephi`` from ``Hr/Hz``.
        - Defined in the ``contains`` section of this module file.
      * - ``sub_E01_cpml_2d_rz_tez_H``
        - CPML magnetic-field entry that updates ``Hr/Hz`` from ``Ephi``.
        - Defined in the ``contains`` section of this module file.
   ...
   .. doxygenfile:: mod_E01_cpml_2d_rz_tez.f90
