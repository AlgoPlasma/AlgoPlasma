...
.. container:: ap-lang ap-lang-zh
   ...
   这个 module wrapper 通过 ``#include`` 汇入 E01 TMz 分量组的 ``Er/Ez`` 与
   ``Ha/Hphi`` 更新核，供上层 ``use mod_E01_fdtd_2d_rz_tmz`` 后直接调用。
   ...
   .. list-table::
      ...
      * - 入口 / 文件
        - 角色
        - 关系
      * - ``sub_E01_fdtd_2d_rz_tmz_H``
        - 用 ``Er/Ez`` 更新 ``Ha/Hphi`` 的磁场入口。
        - 由 Fortran 预处理 ``#include`` 汇入 module。
      * - ``sub_E01_fdtd_2d_rz_tmz_E``
        - 用 ``Ha/Hphi`` 更新 ``Er/Ez`` 的电场入口。
        - 由 Fortran 预处理 ``#include`` 汇入 module。
...
.. container:: ap-lang ap-lang-en
   ...
   This module wrapper includes the E01 TMz component group kernels for updating ``Er/Ez`` and ``Ha/Hphi``.
   ...
   .. list-table::
      ...
      * - Entry / File
        ...
        - Relation
      * - ``sub_E01_fdtd_2d_rz_tmz_H``
        - Magnetic-field entry that updates ``Ha/Hphi`` from ``Er/Ez``.
        - Included into the module by the Fortran preprocessor.
      * - ``sub_E01_fdtd_2d_rz_tmz_E``
        - Electric-field entry that updates ``Er/Ez`` from ``Ha/Hphi``.
        - Included into the module by the Fortran preprocessor.
   ...
   .. doxygenfile:: mod_E01_fdtd_2d_rz_tmz.f90
