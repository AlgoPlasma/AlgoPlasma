...
.. container:: ap-lang ap-lang-zh
   ...
   ``sub_F01_par_load`` 读取粒子数组 ``par``，文件名由 ``label``、``it`` 和 MPI rank 或格式标签决定。
...
.. container:: ap-lang ap-lang-en
   ...
   ``sub_F01_par_load`` dispatch particle loading to a format-specific reader.
   ...
   .. list-table::
      ...
      * - ``label``
        - in
        - scalar or caller-provided array
        - character(*), input directory and file name prefix forwarded to the chosen load
          routine.
      * - ``it``
        - in
        - scalar or caller-provided array
        - integer, time step or iteration index forwarded to the chosen load routine.
      * - ``np``
        - in
        - scalar or caller-provided array
        - integer, number of particles or records to be loaded on this MPI rank.
      * - ``par``
        - out
        - ``(:,:)``
        - real,dimension(:,:), particle phase-space array of the local MPI rank that receives
          loaded data.
   ...
   .. doxygenfile:: sub_F01_par_load.f90
