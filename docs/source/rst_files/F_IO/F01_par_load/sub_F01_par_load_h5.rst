...
.. container:: ap-lang ap-lang-zh
   ...
   ``sub_F01_par_load_h5`` 读取粒子数组 ``par``，文件名由 ``label``、``it`` 和 MPI rank 或格式标签决定。
...
.. container:: ap-lang ap-lang-en
   ...
   ``sub_F01_par_load_h5`` load particle data from per-rank HDF5 files.
   ...
   .. list-table::
      ...
      * - ``label``
        - in
        - scalar or caller-provided array
        - Input directory and file name prefix; must not contain ``/``.
      * - ``it``
        - in
        - scalar or caller-provided array
        - Current time step or iteration index encoded in the file name.
      * - ``np``
        - in
        - scalar or caller-provided array
        - Number of particle columns to read from dataset ``par``.
      * - ``par``
        - out
        - ``(:,1:np)``
        - 2D buffer that receives ``par(:,1:np)``.
   ...
   .. doxygenfile:: sub_F01_par_load_h5.f90
