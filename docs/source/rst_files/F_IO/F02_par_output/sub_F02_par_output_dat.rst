...
.. container:: ap-lang ap-lang-zh
   ...
   ``sub_F02_par_output_dat`` 输出粒子数组 ``par``，文件名由 ``label``、``it`` 和 MPI rank 或格式标签决定。
...
.. container:: ap-lang ap-lang-en
   ...
   ``sub_F02_par_output_dat`` write particle data to per-rank ASCII files.
   ...
   .. list-table::
      ...
      * - ``np``
        - in
        - scalar or caller-provided array
        - integer, number of particles owned by the current MPI rank.
      * - ``par``
        - in
        - ``(:,:)``
   ...
   .. doxygenfile:: sub_F02_par_output_dat.f90
