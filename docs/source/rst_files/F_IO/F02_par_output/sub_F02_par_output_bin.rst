...
.. container:: ap-lang ap-lang-zh
   ...
   ``sub_F02_par_output_bin`` 输出粒子数组 ``par``，文件名由 ``label``、``it`` 和 MPI rank 或格式标签决定。
...
.. container:: ap-lang ap-lang-en
   ...
   ``sub_F02_par_output_bin`` write particle data to per-rank binary files.
   ...
   .. list-table::
      ...
      * - ``it``
        - in
        - scalar or caller-provided array
        - integer, current iteration index encoded in the file name.
      * - ``np``
        - in
        - scalar or caller-provided array
        - integer, number of particle columns to write.
      * - ``par``
        - in
        - ``(:,:)``
   ...
   .. doxygenfile:: sub_F02_par_output_bin.f90
