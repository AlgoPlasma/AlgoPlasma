...
.. container:: ap-lang ap-lang-zh
   ...
   ``sub_F02_par_output_h5`` 输出粒子数组 ``par``，文件名由 ``label``、``it`` 和 MPI rank 或格式标签决定。
   ...
   实现通过 HDF5 数据集读写数组，构建时必须启用 HDF5 相关宏和链接选项。
...
.. container:: ap-lang ap-lang-en
   ...
   ``sub_F02_par_output_h5`` write particle data to per-rank HDF5 files.
   ...
   .. list-table::
      ...
      * - ``label``
        - in
        - scalar or caller-provided array
        - character(*), output directory and file name prefix; should be a simple name and
          must not contain ``/``.
      * - ``it``
        - in
        - scalar or caller-provided array
        - integer, time step index encoded in the file name and stored as dataset attribute
          ``it``.
      * - ``np``
        - in
        - ``(:,p)``
        - integer, number of particle columns ``par(:,p)`` written for the local MPI rank.
      * - ``par``
        - in
        - ``(:,:)``
        - real,dimension(:,:), particle data array stored column-wise as ``par(:,p)`` for ``p
          = 1..np``.
   ...
   The implementation reads/writes arrays through HDF5 datasets; the build must enable HDF5 macros and link options.

   .. rubric:: Generated API

   .. doxygenfile:: sub_F02_par_output_h5.f90
