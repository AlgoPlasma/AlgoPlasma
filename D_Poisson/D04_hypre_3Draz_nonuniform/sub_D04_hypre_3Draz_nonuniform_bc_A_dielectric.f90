!> @file sub_D04_hypre_3Draz_nonuniform_bc_A_dielectric.f90
...
!> cylindrical `(r,alpha,z)` cell-centered nonuniform grid.
!>
...
!>
!> By convention, only faces with `bc_type(face) == 4` are processed here,
!> corresponding to
!> `4 = BC_DIELECTRIC`.
!>
...
subroutine sub_D04_hypre_3Draz_nonuniform_bc_A_dielectric( &
    il,iu,A_values,RHS,bc_type,sr_lo,sr_hi,sa_lo,sa_hi,sz_lo,sz_hi)

    implicit none

    integer,dimension(1:3),intent(in) :: il,iu
    real,dimension(:),intent(inout) :: A_values,RHS
    integer,dimension(1:6),intent(in) :: bc_type

    real,dimension(il(2)-1:iu(2),il(3)-1:iu(3)), &
        optional,intent(in) :: sr_lo,sr_hi
    real,dimension(il(1)-1:iu(1),il(3)-1:iu(3)), &
        optional,intent(in) :: sa_lo,sa_hi
    real,dimension(il(1)-1:iu(1),il(2)-1:iu(2)), &
        optional,intent(in) :: sz_lo,sz_hi
    ...
    do i = il(1),iu(1)
        ...
        if (bc_type(1) == BC_DIELECTRIC .and. i == il(1)) then
        ...
        if (bc_type(2) == BC_DIELECTRIC .and. i == iu(1)) then
        ...
        ! averaged dielectric/surface-charge contribution to the RHS.
        if (bc_type(3) == BC_DIELECTRIC .and. j == il(2)) then
        ...
        ! signed dielectric/surface-charge contribution to the RHS.
        if (bc_type(4) == BC_DIELECTRIC .and. j == iu(2)) then
        ...
        ! averaged dielectric/surface-charge contribution to the RHS.
        if (bc_type(5) == BC_DIELECTRIC .and. k == il(3)) then
        ...
        ! signed dielectric/surface-charge contribution to the RHS.
        if (bc_type(6) == BC_DIELECTRIC .and. k == iu(3)) then
...
end subroutine sub_D04_hypre_3Draz_nonuniform_bc_A_dielectric