!> @file sub_D03_hypre_3Draz_uniform_A_mpi.f90
...
!> @details
!> This MPI-local version uses owned-cell index ranges for the local
!> subdomain. The three grid spacings `dr`, `da`, and `dz` are uniform
!> scalars, and `rface_lo` gives the physical radial position of the left
!> radial face of the local owned box.
!>
...
!> if a neighbor exists, the corresponding off-process stencil coefficient
!> is assembled; otherwise the face is treated as a physical boundary and
!> `bc_type` / `bc_value` are applied.
!>
...
!> the solver layer. In this routine, only the `alpha` and `z` directions
!> may be periodic. Radial periodicity is explicitly disallowed.
!>
!> For periodic `alpha` or `z`, if the current cell lies on the local seam
!> and no neighbor rank is present, the same uniform-grid coupling
!> coefficient is assembled through the fallback periodic path.
...
!> `r,alpha,z` on this MPI rank.
!> @param[in] rface_lo: real scalar, physical radial face coordinate at
...
subroutine sub_D03_hypre_3Draz_uniform_A_mpi( &
    ...
    do i = il_loc(1),iu_loc(1)
        ...
        ! Construct the local radial geometry for the current owned cell.
        rface_m = rface_lo + real(i-il_loc(1))*dr
        rface_p = rface_m + dr
...
end subroutine sub_D03_hypre_3Draz_uniform_A_mpi