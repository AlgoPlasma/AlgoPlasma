program main
    ...
    ! ============================================================
    ! D02 analytic BC suite with arbitrary MPI size.
    !
    ! Compared with the original 4-rank test:
    !   1. Supports arbitrary mpi_n.
    !   2. Builds a 2D Cartesian-like decomposition in x-y, full z.
    !   3. Keeps MPI subdomain interfaces as bc=0 inner boundaries.
    !   4. Keeps the original default real declarations.
    !   5. No timing output.
    ! ============================================================
    ...
    real,parameter :: outflow_r0(1:3) = (/0.5*real(nx), 0.5*real(ny), 0.5*real(nz)/)
...
contains
    ...
    subroutine run_linear_case(axis_name, dielectric_axis, bc_global, case_title, filename)
        ...
        call compute_and_report(case_title, filename)
    ...
    subroutine run_case_outflow()
        ...
        call compute_and_report('case 4: constant field with D02 outflow on all faces', fname)
    ...
    subroutine solve_linear_case(dielectric_axis, bc, phibc)
        ...
        case ('x')
            call assemble_and_solve(bc, phibc, 0.0, zero_vec, use_sx1=.true., use_sx2=.true.)
        case ('y')
            call assemble_and_solve(bc, phibc, 0.0, zero_vec, use_sy1=.true., use_sy2=.true.)
        case ('z')
            call assemble_and_solve(bc, phibc, 0.0, zero_vec, use_sz1=.true., use_sz2=.true.)
        case default
            write(*,*) 'ERROR: unsupported dielectric axis.'
            stop
        end select
    ...
    subroutine compute_and_report(case_title, filename)
        implicit none
        character(len=*),intent(in) :: case_title, filename
