program test_mms_3d_cyl_m0_convergence
    ...
    integer, parameter :: nphi_list(nlev) = (/16, 16, 16/)
    ...
    write(*,'(A)') '=== MMS Convergence: 3D cylindrical m=0 ==='
...
contains
    ...
    subroutine run_level(nr,nphi,nz,dt,nsteps,omega0,l2_out,linf_out,axis_linf_out,l2_comb_out,linf_comb_out,axis_linf_comb_out, &
        ...
        if (viz_on) then
            ...
            open(newunit=viz_unit,file='mms_cyl_m0_viz_fine.bin',form='unformatted',access='stream',status='replace')
            write(viz_unit) nr
            write(viz_unit) nz
        ...
        do n = 1, nsteps
            ...
            if (viz_on .and. (mod(n,viz_stride) == 0 .or. n == nsteps)) then
                call write_viz_frame(viz_unit,nr,nphi,nz,dr,dphi,dz,n,t_n,omega0,Ez)
            end if
    ...
    subroutine write_viz_frame(unit,nr,nphi,nz,dr,dphi,dz,step,t_e,omega0,Ez)
        ...
        real, parameter :: phi_n = 0.0
        integer, parameter :: j_slice = 1
        real :: phi_from_grid

        phi_from_grid = (real(j_slice)-1.0)*dphi
        if (abs(phi_from_grid-phi_n) > 1.0e-12) then
            write(*,'(A,1PE12.4)') 'warning: unexpected phi slice in m0 viz, phi=',phi_from_grid
        end if
        ...
        do i = 0, nr
            ...
            r = real(i)*dr
            z = (real(k)+0.5)*dz
            call cyl_m0_exact_and_sources(r,z,t_e,ep,mu,rmax,lz,omega0, &
    ...
    subroutine init_fields(nr,nphi,nz,dr,dz,dt,omega0,Er,Ephi,Ez,Hr,Hphi,Hz)
        ...
        do i = 0, nr
            r = (real(i)+0.5)*dr
            z = real(k)*dz
            call cyl_m0_exact_and_sources(r,z,0.0,ep,mu,rmax,lz,omega0, &
                er0,ephi0,ez0,hr0,hphi0,hz0,ser,sephi,sez,shr,shphi,shz)
            Er(i,j,k) = er0
            ...
            r = real(i)*dr
            z = (real(k)+0.5)*dz
            call cyl_m0_exact_and_sources(r,z,-0.5*dt,ep,mu,rmax,lz,omega0, &
                er0,ephi0,ez0,hr0,hphi0,hz0,ser,sephi,sez,shr,shphi,shz)
            Hr(i,j,k) = hr0
    ...
    subroutine add_h_source(nr,nphi,nz,dr,dz,dt,t_src,omega0,Hr,Hphi,Hz)
        ...
        do i = 0, nr-1
            r = real(i)*dr
            z = (real(k)+0.5)*dz
            call cyl_m0_exact_and_sources(r,z,t_src,ep,mu,rmax,lz,omega0, &
                er0,ephi0,ez0,hr0,hphi0,hz0,ser,sephi,sez,shr,shphi,shz)
    ...
    subroutine enforce_e_boundaries(nr,nphi,nz,dr,dz,t_e,omega0,Er,Ephi,Ez)
        ...
        do i = 0, nr
            r = (real(i)+0.5)*dr
            call cyl_m0_exact_and_sources(r,0.0,t_e,ep,mu,rmax,lz,omega0, &
                er0,ephi0,ez0,hr0,hphi0,hz0,ser,sephi,sez,shr,shphi,shz)
            Er(i,j,0) = er0
            call cyl_m0_exact_and_sources(r,lz,t_e,ep,mu,rmax,lz,omega0, &
                er0,ephi0,ez0,hr0,hphi0,hz0,ser,sephi,sez,shr,shphi,shz)
            Er(i,j,nz) = er0
    ...
    subroutine compute_errors(nr,nphi,nz,dr,dz,dphi,t_e,t_h,omega0,Er,Ephi,Ez,Hr,Hphi,Hz, &
        ...
        do i = 1, nr-1
            r = real(i)*dr
            z = (real(k)+0.5)*dz
            call cyl_m0_exact_and_sources(r,z,t_h,ep,mu,rmax,lz,omega0, &
                er0,ephi0,ez0,hr0,hphi0,hz0,ser,sephi,sez,shr,shphi,shz)
...
end program test_mms_3d_cyl_m0_convergence
