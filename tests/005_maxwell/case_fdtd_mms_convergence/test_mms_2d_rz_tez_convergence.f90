program test_mms_2d_rz_tez_convergence
    ...
    character(len=8), parameter :: comp_names(ncomp) = [character(len=8) :: 'Ephi', 'Hr', 'Hz']
    ...
    write(*,'(A)') '=== MMS Convergence: 2D RZ TEz ==='
...
contains
    ...
    subroutine run_level(nr,nz,dt,nsteps,omega0,l2_out,linf_out,axis_linf_out,l2_comb_out,linf_comb_out,axis_linf_comb_out, &
        ...
        call init_fields(nr,nz,dr,dz,dt,omega0,Ephi,Hr,Hz)

        viz_open = .false.
        if (viz_on) then
            viz_stride = max(1,nsteps/90)
            open(newunit=viz_unit,file='mms_rz_tez_viz_fine.bin',form='unformatted',access='stream',status='replace')
            write(viz_unit) nr
            write(viz_unit) nz
            write(viz_unit) viz_stride
            call write_viz_frame(viz_unit,nr,nz,dr,dz,0,0.0,omega0,Ephi)
            viz_open = .true.
        end if
        ...
        do n = 1, nsteps
            ...
            call add_h_source(nr,nz,dr,dz,dt,t_n,omega0,Hr,Hz)
            call enforce_h_ghosts(nr,nz,dr,dz,t_n+0.5*dt,omega0,Hr,Hz)

            call sub_E01_fdtd_2d_rz_tez_E(0,nr,0,nz,0,nr-1,1,nz-1,Ephi,Hr,Hz,dt,dr,dz,ep)
            call add_e_source(nr,nz,dr,dz,dt,t_n+0.5*dt,omega0,Ephi)
            call enforce_e_ghosts(nr,nz,dr,dz,t_n+dt,omega0,Ephi)
            t_n = t_n + dt

            if (viz_on .and. (mod(n,viz_stride) == 0 .or. n == nsteps)) then
                call write_viz_frame(viz_unit,nr,nz,dr,dz,n,t_n,omega0,Ephi)
            end if
        ...
        call compute_errors(nr,nz,dr,dz,t_n,t_n-0.5*dt,omega0,Ephi,Hr,Hz, &
    ...
    subroutine write_viz_frame(unit,nr,nz,dr,dz,step,t_e,omega0,Ephi)
        implicit none
        integer, intent(in) :: unit, nr, nz, step
        real, intent(in) :: dr, dz, t_e, omega0
        real, intent(in) :: Ephi(0:nr,0:nz)
        integer :: i, k
        real :: r, z
        real :: ephi0, hr0, hz0, sephi, shr, shz
        real :: Ephi_exact(0:nr,0:nz)

        do k = 0, nz
        do i = 0, nr
            r = real(i)*dr
            z = real(k)*dz
            call rz_tez_exact_and_sources(r,z,t_e,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Ephi_exact(i,k) = ephi0
        end do
        end do
        ...
        write(unit) t_e
        write(unit) Ephi
        write(unit) Ephi_exact
    end subroutine write_viz_frame


    subroutine init_fields(nr,nz,dr,dz,dt,omega0,Ephi,Hr,Hz)
        implicit none
        integer, intent(in) :: nr, nz
        ...
        do i = 0, nr
            r = real(i)*dr
            z = real(k)*dz
            call rz_tez_exact_and_sources(r,z,0.0,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Ephi(i,k) = ephi0

            z = (real(k)+0.5)*dz
            call rz_tez_exact_and_sources(r,z,-0.5*dt,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Hr(i,k) = hr0
    ...
    subroutine add_h_source(nr,nz,dr,dz,dt,t_src,omega0,Hr,Hz)
        implicit none
        integer, intent(in) :: nr, nz
        real, intent(in) :: dr, dz, dt, t_src, omega0
        real, intent(inout) :: Hr(0:nr,0:nz), Hz(0:nr,0:nz)
        integer :: i, k
        real :: r, z, ephi0, hr0, hz0, sephi, shr, shz

        do k = 0, nz-1
        do i = 0, nr-1
            r = real(i)*dr
            z = (real(k)+0.5)*dz
            call rz_tez_exact_and_sources(r,z,t_src,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Hr(i,k) = Hr(i,k) + dt*shr

            r = (real(i)+0.5)*dr
            z = real(k)*dz
            call rz_tez_exact_and_sources(r,z,t_src,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Hz(i,k) = Hz(i,k) + dt*shz
        end do
        end do
    end subroutine add_h_source


    subroutine add_e_source(nr,nz,dr,dz,dt,t_src,omega0,Ephi)
        implicit none
        integer, intent(in) :: nr, nz
        real, intent(in) :: dr, dz, dt, t_src, omega0
        ...
        do i = 0, nr-1
            ...
            z = real(k)*dz
            call rz_tez_exact_and_sources(r,z,t_src,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Ephi(i,k) = Ephi(i,k) + dt*sephi
        end do
        end do
    end subroutine add_e_source


    subroutine enforce_h_ghosts(nr,nz,dr,dz,t_h,omega0,Hr,Hz)
        implicit none
        integer, intent(in) :: nr, nz
        ...
        do k = 0, nz
            r = real(nr)*dr
            z = (real(k)+0.5)*dz
            call rz_tez_exact_and_sources(r,z,t_h,ep,mu,rmax,lz,omega0,ephi0,hr0,hz0,sephi,shr,shz)
            Hr(nr,k) = hr0
    ...
    subroutine enforce_e_ghosts(nr,nz,dr,dz,t_e,omega0,Ephi)
        implicit none
        integer, intent(in) :: nr, nz
        real, intent(in) :: dr, dz, t_e, omega0
        real, intent(inout) :: Ephi(0:nr,0:nz)
    ...
    subroutine compute_errors(nr,nz,dr,dz,t_e,t_h,omega0,Ephi,Hr,Hz,l2c,linfc,axis_linfc,l2all,linfall,axis_linfall, &
        ...
        l2e = sqrt(sumsq(1)/max(sumw(1),1.0e-30))
        l2h = sqrt((sumsq(2)+sumsq(3))/max(sumw(2)+sumw(3),1.0e-30))
        linfe = linfc(1)
        linfh = max(linfc(2),linfc(3))
...
end program test_mms_2d_rz_tez_convergence
