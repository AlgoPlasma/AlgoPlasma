program test_pulse_2d_rz_tez
    ...
    real :: axis_near_ephi_max, axis_near_hz_max
    ...
    call compute_metrics(nr,nz,dr,dz,ep,mu,axis_band_cells,Ephi,Hr,Hz, &
        ...
        axis_near_ephi_max,axis_near_hz_max,local_nan)
    ...
    write(*,'(A)') '=== Pulse Long-Run Stability: 2D RZ TEz ==='
    ...
    write(*,'(A)') '# step,time,pulse_on,max_abs_E,max_abs_H,total_energy,axis_band_max,first_ring_max,' // &
        'i0_active_max,i1_first_ring_max,axis_near_Ephi_max,axis_near_Hz_max'
    ...
    call write_monitor_row(0,0.0,pulse_on,max_abs_e,max_abs_h,total_energy,axis_band_max,first_ring_max, &
        i0_active_max,i1_active_max,axis_near_ephi_max,axis_near_hz_max)
    ...
    do n = 1, nsteps
        ...
        if (n <= npulse) call add_pulse_source(n,npulse,pulse_amp,nr,nz,dr,dz,Ephi)
    ...
    write(*,'("SUMMARY_CSV,2D_RZ_TEz,",F5.3,",",I0,",",I0,",",1PE12.4,",",1PE12.4,",",A)') &
...
contains
    ...
    subroutine write_monitor_row(step,time,pulse_on,maxe,maxh,energy,axismax,ringmax,i0max,i1max,eax,hzax)
        implicit none
        integer, intent(in) :: step, pulse_on
        real, intent(in) :: time,maxe,maxh,energy,axismax,ringmax,i0max,i1max,eax,hzax
        write(*,'(*(g0,:,","))') &
            step,time,pulse_on,maxe,maxh,energy,axismax,ringmax,i0max,i1max,eax,hzax
    end subroutine write_monitor_row


    subroutine add_pulse_source(n,npulse,amp,nr,nz,dr,dz,Ephi)
        implicit none
        ...
        do i = 2, nr-2
            r = real(i)*dr
            z = real(k)*dz
            phase = sin(2.0*acos(-1.0)*(z-z0)/lz)
            shape = exp(-((r-r0)/wr)**2 - ((z-z0)/wz)**2) * phase
            Ephi(i,k) = Ephi(i,k) + pulse_t*shape
        end do
        end do
    ...
    subroutine compute_metrics(nr,nz,dr,dz,ep,mu,axis_cells,Ephi,Hr,Hz, &
        max_abs_e,max_abs_h,total_energy,axis_band_max,first_ring_max,i0_active_max,i1_active_max, &
        axis_near_ephi_max,axis_near_hz_max,has_nan)
        implicit none
        ...
        real, intent(out) :: i0_active_max, i1_active_max, axis_near_ephi_max, axis_near_hz_max
        ...
        do i = 1, nr-1
            ...
            if (.not. ieee_is_finite(v)) then
                ...
                if (i <= 1) axis_near_ephi_max = max(axis_near_ephi_max,av)
                r = real(i)*dr
                w = r*dr*dz
...
end program test_pulse_2d_rz_tez
