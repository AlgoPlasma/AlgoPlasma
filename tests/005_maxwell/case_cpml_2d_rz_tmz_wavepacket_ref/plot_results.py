...
def read_case_info():
    ...
    for line in Path("case_info.dat").read_text().splitlines():
        if "npml npml_ref" in line:
            ...
            info["dr"] = vals[0]
            info["dz"] = vals[1]
        elif "zmin_interior" in line:
            vals = numbers_from_line(line)
            info["zmin_interior"] = vals[0]
        elif "zmin_cpml" in line:
            vals = numbers_from_line(line)
            info["zmin_cpml"] = vals[0]
...
def probe_index(case, info):
    dr = info["dr"]
    dz = info["dz"]
    r_probe = 0.5*info["nr_interior"]*dr
    z_probe = info["zmin_interior"] + 0.5*info["nz_interior"]*dz
    ...
    if case == "z_plus":
        ...
        r_probe = info["probe_margin"]

    ir = int(np.rint(r_probe/dr))
...
def plot_probes():
    ...
    for ax, case in zip(axes.ravel(), CASES):
        ...
        ax.plot(data[:, 0], data[:, 2], lw=0.9, label="large reference", alpha=0.8)
        ax.set_title(case)
        ax.set_xlabel("step")
        ax.set_ylabel("probe E")
...
def main():
    plot_probes()
    plot_field_snapshots("er", "Er")
    plot_field_snapshots("ez", "Ez")
    plot_field_snapshots("ha", "Ha")
    print("Wrote probe_compare.png, probe_error_db.png, and field snapshot PNGs")
