program test_3d_cyl_m1_single_step
    ...
    real, parameter :: tol_seam = 1.0e-10
    ...
    all_ok = .true.
    call init_cyl_m1_fields(nr,nphi,nz,dr,dz,dphi,Er0,Ephi0,Ez0,Hr0,Hphi0,Hz0)

    write(*,'(A)') '=== 3D Cylindrical FDTD Single-Step Formula Test (m=1, with phi seam) ==='

    call report_init(rep)
    Er_num = Er0; Ephi_num = Ephi0; Ez_num = Ez0
    Hr_num = Hr0; Hphi_num = Hphi0; Hz_num = Hz0
    call sub_E02_fdtd_3d_cylindrical_E(0,nr,0,nphi+1,0,nz,1,nr-1,1,nphi,1,nz-1, &
        Er_num,Ephi_num,Ez_num,Hr_num,Hphi_num,Hz_num,dt,dr,dphi,dz,ep)

    Er_ref = Er0; Ephi_ref = Ephi0; Ez_ref = Ez0
    call ref_update_cyl_E(nr,nphi,nz,1,nr-1,1,nphi,1,nz-1, &
    ...
    call write_pgm_2d('err_3d_cyl_m1_E_step_rz_maxphi.pgm',nr,nz,err2d_rz)
    call report_print('E-step (m=1)', rep)
    all_ok = all_ok .and. (rep%n_failed == 0)

    call report_init(rep)
    Er_num = Er0; Ephi_num = Ephi0; Ez_num = Ez0
    Hr_num = Hr0; Hphi_num = Hphi0; Hz_num = Hz0
    call sub_E02_fdtd_3d_cylindrical_H(0,nr,0,nphi+1,0,nz,1,nr-1,1,nphi,0,nz-1, &
        Er_num,Ephi_num,Ez_num,Hr_num,Hphi_num,Hz_num,dt,dr,dphi,dz,mu)

    Hr_ref = Hr0; Hphi_ref = Hphi0; Hz_ref = Hz0
    call ref_update_cyl_H(nr,nphi,nz,1,nr-1,1,nphi,0,nz-1, &
    ...
    call project_max_over_j(nr,nphi+1,nz,err3d,err2d_rz)
    call write_pgm_2d('err_3d_cyl_m1_H_step_rz_maxphi.pgm',nr,nz,err2d_rz)
    call report_print('H-step (m=1)', rep)
    all_ok = all_ok .and. (rep%n_failed == 0)
    ...
    call project_max_over_j(nr,nphi+1,nz,err3d,err2d_rz)
    call write_pgm_2d('err_3d_cyl_m1_full_step_rz_maxphi.pgm',nr,nz,err2d_rz)
    call report_print('Full-step (E then H, m=1)', rep)
    all_ok = all_ok .and. (rep%n_failed == 0)
...
contains
    ...
    subroutine init_cyl_m1_fields(nr0,nphi0,nz0,dr0,dz0,dphi0,Er,Ephi,Ez,Hr,Hphi,Hz)
        implicit none
        ...
        do j = 0, nphi0+1
        do i = 0, nr0
            phi_n = (real(j)-1.0)*dphi0
            phi_h = (real(j)-0.5)*dphi0

            r = (real(i)+0.5)*dr0
            z = real(k)*dz0
            Er(i,j,k) = r*(0.24 + 0.06*r/lr) * cos(phi_n) * sin(2.0*pi*z/lz + 0.12)

            r = real(i)*dr0
            z = real(k)*dz0
            Ephi(i,j,k) = r*(0.21 + 0.05*r/lr) * sin(phi_h) * cos(2.0*pi*z/lz + 0.34)

            r = real(i)*dr0
            z = (real(k)+0.5)*dz0
            Ez(i,j,k) = (0.86 + 0.07*r/lr) * cos(phi_n) * cos(2.0*pi*z/lz + 0.22)

            r = real(i)*dr0
            z = (real(k)+0.5)*dz0
            Hr(i,j,k) = r*(0.16 + 0.04*r/lr) * sin(phi_h + 0.11) * sin(2.0*pi*z/lz + 0.18)

            r = (real(i)+0.5)*dr0
            z = (real(k)+0.5)*dz0
            Hphi(i,j,k) = r*(0.17 + 0.05*r/lr) * cos(phi_n + 0.15) * cos(2.0*pi*z/lz + 0.29)

            r = (real(i)+0.5)*dr0
            z = real(k)*dz0
            Hz(i,j,k) = (0.62 + 0.05*r/lr) * sin(phi_h) * sin(2.0*pi*z/lz + 0.41)
        end do
        end do
        end do
    end subroutine init_cyl_m1_fields
    ...
    subroutine check_cyl_E_domain(nr0,nphi0,nz0,Er_ref,Ephi_ref,Ez_ref,Er_num,Ephi_num,Ez_num,rep)
        ...
        do i = 0, nr0
            tol = tol_interior
            if (j == 1 .or. j == nphi0) tol = tol_seam
            if (i == 0 .or. i == nr0 .or. j == 0 .or. j == nphi0+1 .or. k == 0 .or. k == nz0) tol = tol_boundary
...
end program test_3d_cyl_m1_single_step
