! Force-free high-gamma ExB drift test.
! Sets up E = -vy*Bz (Lorentz force cancels exactly) with gamma = 20.
! The particle should drift in y at constant velocity with x = 0 always.
! HC maintains x ≈ 0; the Boris pusher drifts by ~2321 under the same setup
! (see WarpX Examples/Tests/particle_pusher for reference).
subroutine sub_case02_exb_drift(nstep,dt,qm,B0,Ex,v0,v_init,E0,fname)
    ...
    ! v_drift = E×B / B² = -Ex/B0  (exact for E < cB)
    vdrift = -Ex/B0
    v      = (/0.0, vdrift, 0.0/)
    r      = 0.0
    ...
    close(unit)
    write(*,'("Case ",a6,": max|v-v_ana| = ",es12.4,"   max|r-r_ana| = ",es12.4)') 'HC_g20', err_v_max, err_r_max
    write(*,'("         final |x|         = ",es12.4,"   (Boris ~2321, tol 0.001)")') abs(r(1))

end subroutine sub_case02_exb_drift
