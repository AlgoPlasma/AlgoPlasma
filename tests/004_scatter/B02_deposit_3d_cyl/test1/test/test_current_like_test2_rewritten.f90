!> @file test_current_like_test2_rewritten.f90
!> @brief Paper-style current deposition test with corrected output layout.
!> @details This test follows the logic of the current test in
!>          ``Rigorously conservative charge and current deposition in
!>          3D cylindrical PIC``. It deposits ``n0``, advances particles by
!>          one small step with a constant velocity, deposits ``Jr``,
!>          ``Jphi`` and ``Jz``, deposits ``n1``, and writes four map files.
!>          The output coordinates are written in the same physical layout as
!>          the paper figures: horizontal axis is the z grid index and vertical
!>          axis is the signed radial grid index. In particular, ``Jr`` is
!>          written on half radial indices ``±(i+1/2)`` and no artificial
!>          ``r=0`` column is inserted.
!> @author ZhouZhijun （2026/04/16)
program test_current_like_test2_rewritten
    ...
    real :: ur,uphi,uz,rp0,phip0,zp0,rp1,phip1,zp1
    real :: nbar0,expected_jr,expected_jz,expected_jphi
    real :: div_r,div_phi,div_z,err_local,denom,xi,yi
    real :: nanv
    ...
    do ip = 1,Np
        ...
        call sub_B02_deposit_charge_3d_cyl(rp0,phip0,zp0,1.0,1.0,dr,dphi,dz, &
            Nr,Nphi,Nz,n0_arr)
        ...
        call sub_B02_deposit_current_3d_cyl(rp0,phip0,zp0,rp1,phip1,zp1,1.0,1.0, &
            dr,dphi,dz,Nr,Nphi,Nz,dt,jr,jphi,jz)

        call sub_B02_deposit_charge_3d_cyl(rp1,phip1,zp1,1.0,1.0,dr,dphi,dz, &
            Nr,Nphi,Nz,n1_arr)
    end do
    ...
    open(10,file='current_jr_map.dat',status='replace')
    do i = Nr - 1,0,-1
        yi = -(real(i) + 0.5)
        do k = 0,Nz
            xi = real(k)
    ...
    open(11,file='current_jphi_map.dat',status='replace')
    do i = Nr,1,-1
        yi = -real(i)
    ...
    open(12,file='current_jz_map.dat',status='replace')
    do i = Nr,1,-1
        yi = -real(i)
        do k = 0,Nz - 1
            xi = real(k)
    ...
    open(13,file='current_error_map.dat',status='replace')
    do i = Nr - 1,1,-1
        yi = -real(i)
        do k = 1,Nz - 1
            xi = real(k)
    ...
    print *, 'Paper-style current test finished.'
    print *, 'Np used = ',Np
    print *, 'Output:'
    print *, '  current_jr_map.dat'
    print *, '  current_jphi_map.dat'
    print *, '  current_jz_map.dat'
    print *, '  current_error_map.dat'

    deallocate(n0_arr,n1_arr,jr,jphi,jz)

end program test_current_like_test2_rewritten
