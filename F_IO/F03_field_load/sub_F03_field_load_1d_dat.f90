!> @file sub_F03_field_load_1d_dat.f90
!> @brief Load a 3D cell-centered field stored as a 1D vector from per-rank
!>   ASCII files.
!>
!> @details
!> Each MPI rank reads its local field vector ``F`` from an ASCII ``.dat``
!> file under directory ``trim(label)``.
!>
...
!> ``sub_F04_field_output_1d_dat``:
!> the first six lines store ``il(1:3)`` then ``iu(1:3)``
!> (one integer per line), followed by ``N`` real values ``F(1:N)``
!> written in nested loop order ``k``, ``j``, ``i`` while advancing the
!> 1D index from 1 to ``N``.
!>
...
subroutine sub_F03_field_load_1d_dat(label,it,il,iu,F)
    ...
    if (ios /= 0) then
        ...
        write(*,'(A)') trim(iomsg)
        call mpi_abort(mpi_comm_world, 2001, ierr)
    end if
    ...
    if (ncell <= 0) then
        ...
        close(unit)
        call mpi_abort(mpi_comm_world, 2002, ierr)
    end if
    ...
    if (ios /= 0) then
        ...
        close(unit)
        call mpi_abort(mpi_comm_world, 2003, ierr)
    end if
    ...
    if (ios /= 0) then
        ...
        write(*,'(A)') trim(iomsg)
        close(unit)
        call mpi_abort(mpi_comm_world, 2004, ierr)
    end if
...
900 continue
    write(*,'(A,I0,2A)') 'ERROR(rank=', mpi_i, &
        '): failed reading header from: ', trim(filename)
    write(*,'(A)') trim(iomsg)
    close(unit)
    call mpi_abort(mpi_comm_world, 2005, ierr)

end subroutine sub_F03_field_load_1d_dat
