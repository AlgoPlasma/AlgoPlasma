!> @file sub_F03_field_load_3d_bin.f90
!> @brief Load a 3D cell-centered field from per-rank binary files.
!>
!> @details
!> Each MPI rank reads its local subdomain array ``F`` from an unformatted,
...
!> then all values of ``F`` in Fortran array element order
!> (column-major, with the first index varying fastest).
!>
...
!>   previously written by ``sub_F04_field_output_3d_bin`` without
...
subroutine sub_F03_field_load_3d_bin(label,it,il,iu,F)
    ...
    if (ios /= 0) then
        ...
        write(*,'(A)') trim(iomsg)
        call mpi_abort(mpi_comm_world,2301,ierr)
    end if
    ...
    if (ios /= 0) then
        ...
        write(*,'(A)') trim(iomsg)
        close(unit)
        call mpi_abort(mpi_comm_world,2302,ierr)
    end if
    ...
    if (ios /= 0) then
        ...
        write(*,'(A)') trim(iomsg)
        close(unit)
        call mpi_abort(mpi_comm_world,2303,ierr)
    end if
    ...
    if (any(ilf /= il) .or. any(iuf /= iu)) then
        ...
        close(unit)
        call mpi_abort(mpi_comm_world,2304,ierr)
    end if
    ...
    if (ios /= 0) then
        ...
        write(*,'(A)') trim(iomsg)
        close(unit)
        call mpi_abort(mpi_comm_world,2305,ierr)
    end if

    close(unit)

end subroutine sub_F03_field_load_3d_bin
